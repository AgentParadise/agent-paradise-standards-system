...
## 2. Directory (RET01-dir-not-found)

A directory MUST exist at `docs.retrospectives.directory` (default: `docs/retrospectives/`).

Diagnostic: `RET01-dir-not-found` (error). Hint: "Create the directory at `<directory>` or set `docs.retrospectives.disable = true` in `.apss/config.toml`."
...
## 3. Naming (RET01-naming-mismatch, RET01-invalid-naming-regex)
...
**Default pattern:** `RET-\d{3,5}-[a-zA-Z0-9-]+\.md`

**Examples:**

- `RET-001-q1-launch.md`
- `RET-042-deployment-failure-postmortem.md`
- `RET-10000-five-year-retro.md`

The numeric component MUST support 3 to 5 digits so the project can grow past 1000 retros without changing the standard. (Mirrors the ADR01 rule for consistency.)

A retrospective file MUST be reachable from the directory's `## Index` section once committed.

Diagnostics:
- `RET01-naming-mismatch` (error): a file does not match the regex.
- `RET01-invalid-naming-regex` (error): the configured regex itself is not valid. Hint: "Check `docs.retrospectives.naming_pattern` in `.apss/config.toml`."

## 4. Frontmatter (RET01-frontmatter-missing, RET01-frontmatter-field-missing)
...
## 5. Required sections (RET01-missing-section)
...
Diagnostic: `RET01-missing-section` (warning) with the missing section name in the hint.

## 6. Lifecycle status (RET01-invalid-status)
...
Diagnostic: `RET01-invalid-status` (error).

## 7. Append-only history (RET01-history-modified)

The validator MUST, when invoked in `Changed` scope (Section 9.2 of the parent spec), inspect every retro file that appears in the staged change set and emit `RET01-history-modified` (error) when an existing retro's content has been modified outside an appended footnote region.
...
| `RET01-naming-mismatch` | error | File does not match the naming pattern. |
...
| `RET01-frontmatter-field-missing` | error | Required frontmatter field absent. |
| `RET01-invalid-status` | error | `status` is not one of the allowed values. |
| `RET01-missing-section` | warning | Retro missing a required section heading. |
| `RET01-history-modified` | error | An existing retro was modified outside an append region. |
