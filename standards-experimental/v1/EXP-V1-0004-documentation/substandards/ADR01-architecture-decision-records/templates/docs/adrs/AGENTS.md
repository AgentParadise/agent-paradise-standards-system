...
This file is the canonical agent-context file for this directory. Per the
project's symlink convention, `CLAUDE.md` and `GEMINI.md` in the same
directory are symlinks to this file, so every agent gets the same
guidance from a single source.
