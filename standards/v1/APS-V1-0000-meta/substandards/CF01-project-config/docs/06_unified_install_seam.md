...
pub struct InstallContext<'a> {
    ...
    pub apss_yaml: &'a Path,
    pub resolved_config: serde_yaml::Value,
...
The migration window from EXP-V1-0004's `.apss/config.toml` and
CF01's older `apss.toml` to the unified apss.yaml manifest is
specified in the migration note attached to `01_spec.md`. Per
brief decision 1, the `.apss/` dotdir is retained for generated
artifacts (lockfile resolution outputs, install reports) but is
never configuration.

External consumers MAY use a shim during the window: if both
`apss.yaml` and `apss.toml` exist at the project root, the installer
MUST fail with `CF_DUAL_MANIFEST` and require the consumer to remove
the old file. There is no auto-merge; explicit removal is the only
way to commit to the new manifest.

| Code | Severity | Rule |
|------|----------|------|
| `CF_DUAL_MANIFEST` | Error | Both `apss.yaml` and `apss.toml` (or `.apss/config.toml`) exist at the project root. |
