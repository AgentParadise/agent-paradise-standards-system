...
pub fn generate_build_crate(
    ...
    generate_cargo_toml(config, &cargo_path)?;
...
fn generate_cargo_toml(config: &ResolvedProjectConfig, path: &Path) -> Result<(), CodegenError> {
    ...
    content.push_str("aps-core = { version = \"0.1\" }\n");
    ...
    for standard in config.standards.values() {
        ...
        content.push_str(&format!(
            "{} = {{ version = \"{}\" }}\n",
            standard.crate_name, standard.version_req
        ));
    }
...
fn generate_main_rs(config: &ResolvedProjectConfig, path: &Path) -> Result<(), CodegenError> {
    ...
    content.push_str(
        "    let mut runner = ProjectRunner::from_config_file(Path::new(\"apss.toml\"))\n",
    );
...
mod tests {
    ...
    fn test_generate_build_crate() {
        ...
        let files = generate_build_crate(&config, temp.path()).unwrap();
    ...
    fn test_generate_skips_disabled() {
        ...
        generate_build_crate(&config, temp.path()).unwrap();
