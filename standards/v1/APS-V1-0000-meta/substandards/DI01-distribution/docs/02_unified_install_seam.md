...
to `apss.toml` in `01_spec.md` are superseded by `apss.yaml` per the
...
- A resolved-config blob at `.apss/install/resolved.yaml` whose schema
...
`<binary> install --locked` MUST fail at the resolve step if the
...
2. Either keep the previous composed binary on disk as
   `.apss/bin/<binary>.prev` and invoke its `uninstall_<slug>`
...
|---------|--------|-------------|
| `<binary> init` | REFINED | Creates apss.yaml (not apss.toml) and an initial `.gitignore` block for `.apss/`. |
| `<binary> install` | REFINED | Drives the full resolve and install flow described in §3. |
| `<binary> install --locked` | REFINED | Resolve must produce no lockfile change; per-standard install still runs. |
| `<binary> install --offline` | REFINED | Resolve uses cached crates only; per-standard install still runs (per-standard contracts MUST be offline-safe). |
| `<binary> install --dry-run` | NEW | Runs resolve and computes per-standard install diffs, but writes nothing. Prints the diff. |
| `<binary> install --update <slug>` | REFINED | Updates one standard, runs only that standard's `update()`. |
| `<binary> uninstall <slug>` | NEW | Runs one standard's `uninstall()`. Equivalent to removing the slug from apss.yaml and re-running install, but does not require an edit. |
| `<binary> status` | REFINED | Reports installed versions AND the persisted install reports per standard. |
| `<binary> validate` | REFINED | Delegates to CF01 per the validation delegation protocol. |
| `<binary> config show <slug>` | REFINED | Reads resolved config from `.apss/install/resolved.yaml` so the output matches what the installer actually used. |
| `<binary> config schema <slug>` | REFINED | Reads the per-slug contribution schema (`03_contribution_schema.md` §5.1). |

### 4.2 Removed Surface

`<binary> install` no longer has the responsibility "make
`apss.toml` exist if missing"; that was `<binary> init`'s job and
remains so. Likewise, `install` MUST NOT touch the slug registry
artifact directly; it consumes the artifact (regenerated by
`<binary> v1 generate slug-registry` per
...
project-root/
  ...
  .apss/
    bin/
      <binary>                    # composed binary (DI01 §6)
      <binary>.prev               # optional, for fast-path uninstall (§3.3)
    build/
      Cargo.toml
      src/main.rs                 # generated, includes install_all()
    install/
      resolved.yaml               # resolved manifest blob (§2.2)
...
`<binary> init` if not already present.
...
| Composed binary | DI01 | `.apss/bin/<binary>` and code generation |
...
document use `<binary>` so that the resolution of issue 64 is a
