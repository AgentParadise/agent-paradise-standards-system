...
|---------|--------|-------------|
| `<bootstrap> init` | REFINED | Creates apss.yaml (not apss.toml) and an initial `.gitignore` block for `.apss/`. |
...
| `<bootstrap> config show <slug>` | REFINED | Reads resolved config from `.apss/install/resolved.yaml` so the output matches what the installer actually used. |
