...
2. **Use the CLI**: always use `cargo run -p aps-cli --bin apss-dev -- v1 create standard <slug>` to scaffold new standards
3. **Validate before commit**: run `cargo run -p aps-cli --bin apss-dev -- v1 validate standard <id>` before committing changes
...
1. Create experiments with `cargo run -p aps-cli --bin apss-dev -- v1 create experiment <slug>`
...
cargo run -p aps-cli --bin apss-dev -- v1 create standard my-standard

# Validate your work
cargo run -p aps-cli --bin apss-dev -- v1 validate standard APS-V1-XXXX
