...
- **Validation rules** — What the `apss-dev` CLI checks
...
1. Use `cargo run -p aps-cli --bin apss-dev -- v1 create standard <slug>` to scaffold a new standard
2. Implement the `Standard` trait in your crate
3. Add examples and tests
4. Run `cargo run -p aps-cli --bin apss-dev -- v1 validate standard <id>` to check compliance

### For Standard Consumers

1. Check if the standard you need exists: `apss run --list`
